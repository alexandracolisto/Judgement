...
public class InGameMenu {
	 ...
	 * Variables
	 **************/
	private Font bold = new Font("SERIF", Font.BOLD, 42);
	...
	//Reference blue code block above methods
	public void render(JPanel panel, Graphics2D g2d, int inX, int inY) {
		g2d.drawImage(_background.getImage(), 0, 0, SCREENWIDTH, SCREENHEIGHT+1, panel);
		g2d.setFont(bold);
		g2d.setColor(Color.BLACK);
		int xPosition = (int)(SCREENWIDTH/10.3);//relative to the screen
		int yPosition = (int)(SCREENHEIGHT/2.8);//relative to the screen
		g2d.drawString("Items", xPosition, yPosition);
		yPosition += 100;
		g2d.drawString("Equipment", xPosition, yPosition);
		yPosition += 100;
		g2d.drawString("Magic", xPosition, yPosition);
		yPosition += 100;
		g2d.drawString("Status", xPosition, yPosition);
		yPosition += 100;
		g2d.drawString("Save Game", xPosition, yPosition);
		g2d.setColor(Color.BLACK);
		g2d.drawRect(inX, inY, (int)(SCREENWIDTH/5.3), (int)(SCREENHEIGHT/11.5)); //435,104
		
		if(_option == OPTION.ITEMS){
			g2d.setColor(Color.RED);
			g2d.drawString("Items", 920, 200);
			renderItems(panel, g2d, shownItems, items);
		}
		
		if(_option == OPTION.EQUIPMENT){
			g2d.setColor(Color.RED);
			g2d.drawString("Equipment", 900, 200);
			renderItems(panel, g2d, shownEquipment, equipment);
		}
		
		if(_option == OPTION.MAGIC){
			g2d.setColor(Color.RED);
			g2d.drawString("Magic", 900, 200);
		}
		
		if(_option == OPTION.STATUS){
			g2d.setColor(Color.RED);
			g2d.drawString("Status", 920, 200);
			g2d.drawString("Level: " + level, 600, 300);
			g2d.drawString("Attack: " + attack, 600, 400);
			g2d.drawString("Defense: " + defense, 600, 500);
			g2d.drawString("Health: " + currHealth, 600, 600);
			g2d.drawString("Experience: " + experience + " / " + nextLevel, 600, 700);
		}
		//TODO: need to align with screen size (relatively)
	...
	//Reference blue code block above methods
	public void renderItems(JPanel panel, Graphics2D g2d, Item[] array, LinkedList<Item> list) {
		...
		for(int i = start; i < start + 4; i++){
			...
			g2d.drawString(" x " + new Integer(counts[i]).toString(), 600 + array[i].getName().length() * 37,  400 + y * 110);
			array[i].render(panel, g2d, 600, 340 + y * 110);
			y++;
		}
	}
	...
	public void setCounts(String item, int amount){
		if(item.equals("Potion")){
			counts[0] += amount;
			
		}
		else if(item.equals("Mega Potion")){
			counts[1] += amount;
