...
abstract class Game extends JPanel	 implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	...
	protected transient Thread gameloop;
	...
	public Game(int frameRate, int width, int height) {
		...
		//BufferedImage.TYPE_INT_ARGB_PRE was BufferedImage.TYPE_INT_RGB before.
		//Team A2 changed this bufferImage type and the grame performance increased
		//by a lot. No notable difference was seen in the game, which is good. Only benefits.
		backBuffer = new BufferedImage(screenWidth, screenHeight, BufferedImage.TYPE_INT_ARGB_PRE);
	...
	public void update(Graphics g) {
		...
		if(System.currentTimeMillis() > startTime + 100) {
			...
			drawSprites();
			paintComponent(g);//was paint() before with the frame. Better practice with paintComponent & JPanel
			gameRefreshScreen();
	}
	 ...
	 * 
	 * Override the panel's paintComponent method, draw the backBuffer and sync with the system
	...
	public void paintComponent(Graphics g) {
		...
		g.drawImage(backBuffer, 0, 0, this); //A LOT OF CPU USAGE
		tk.sync();
	}
	 ...
	 ***********************************************************/
	//came from the creator of game. I changed the saving file structure.
	//left here in case you want to use his way of saving instead
	 ...
	 *******************************************************************************/
	//commented out for future usage if you decide to go with the creator's way of saving file
