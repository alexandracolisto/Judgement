...
public class Mob extends AnimatedSprite{
	...
	private void randomPath() {
		...
		//System.out.print(moveDir + " ");
		if(moveDir != DIRECTION.NONE) actualDir = moveDir;
		if(wait <= 0) {
			waitOn = false;
			randomDir = DIRECTION.NONE;
			moveDir = DIRECTION.NONE;
	...
	public void renderMob(int x, int y) {
		g2d.drawImage(getImage(), x + xx, y + yy, getSpriteSize(), getSpriteSize(), panel);
		entity.setX(x + xx);
		entity.setY(y + yy);
	}
	/**
	 * get the current direction for debugging
	 * @return: the direction of the mob in a string
	 */
	...
	/**
	 * Gets the direction of the random NPCs pathways. For some reason,
	 * up and down had to be inverse.
	 * 
	 * NEEDS SOME WORK. IF THE NPC HASN'T MOVE YET, IT WILL ALWAYS RETURN THE
	 * BOTTOM "UP". ONLY WORKS WHEN THE NPC STARTS ITS RANDOM PATH
	 * @return: returns the direction in a string of the random NPC
	 */
	public String getRandomMobDir(){
		if(actualDir == DIRECTION.UP) return "up";
		else if(actualDir == DIRECTION.RIGHT) return "right";
		else if(actualDir == DIRECTION.DOWN) return "down";
		else if(actualDir == DIRECTION.LEFT) return "left";
		
		return "up";
	}
	
	/**
	 * Sets the direction given a string of the NPC/playerMob
	 * @param dir: the direction as a string
	 */
	public void setDirection(String dir){
		if(dir.equals("up")) this.direction = DIRECTION.UP;
		else if(dir.equals("right")) this.direction = DIRECTION.RIGHT;
		else if(dir.equals("left")) this.direction = DIRECTION.LEFT;
		else if(dir.equals("down")) this.direction = DIRECTION.DOWN;
	}
	//set hostile
